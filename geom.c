...
{
    // rotated rectangle(context, top, left, width, height, angle degrees)
    cairo_save(cr);
    cairo_set_source_rgba(cr,  k->r, k->g, k->b, k->a);
    cairo_set_line_width(cr, 5);
    cairo_move_to(cr, c->x - w * 0.5, c->y -h * 0.5);
    cairo_rotate(cr, a * DEG2RAD);
    cairo_rectangle(cr, c->x - w * 0.5, c->y -h * 0.5, w, h);
    cairo_fill(cr);
    cairo_restore(cr);
}
...
int cmp(float a, float b) { return (a > b) - (a < b); }
float sind(float x) { return sin(x * RAD2DEG); } // sin of x in degres .            TO BE CHECKED : * or / ?
float cosd(float x) { return cos(x * RAD2DEG); }
float tand(float x) { return tan(x * RAD2DEG); }
float arclength(float r, float a, float b) { return (fabs(r * RAD2DEG * (b - a)));}
float dist(struct Point a, struct Point b){ return (sqrt(pow(b.x-a.x,2)+pow(b.y-a.y,2)));}
float magn(struct Vec v){ return (sqrt(pow(v.x,2)+pow(v.y,2)));}
float cosv(struct Vec v) { return (v.x / magn(v));}
float sinv(struct Vec v) { return (v.y / magn(v));}
struct Vec vec(struct Point a, struct Point b) {return (struct Vec) {.x = b.x-a.x, .y = b.y-a.y};} // vec from 2 points
